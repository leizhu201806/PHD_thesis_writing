...
透过散射介质光谱成像的仿真参数如表\ref{table:1.1}所示，仿真结果如图\ref{fig:3.6}所示，第一行为成像臂在不同波长光照明情况下，相机1所探测到的散斑；第二行为光谱臂在不同光谱不同波长光照明下，相机2所接收到的散斑；
