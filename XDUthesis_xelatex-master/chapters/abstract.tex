\begin{abstract}
当光透过生物组织、烟尘和云雾等强散射介质时，由于光和介质的相互作用形成散斑，而无法直接观测获得隐藏在散射介质之后的目标信息。波前调制技术和光学传输矩阵测量技术能够通过控制入射光场分布，克服介质的散射作用，实现透过散射介质聚焦或成像。现有的波前调制技术和光学传输矩阵测量存在以下限制：首先，波前调制技术需要复杂的反馈调制过程；其次，光学传输矩阵测量需要较长的测量过程；最后，波前调制技术和光学传输矩阵测量难以通过非入侵形式实现。因此它们很难满足实用应用的需求，如生物医学成像等应用。
...
（1）针对目前已有散斑相关成像和光谱传输矩阵的光谱重建所存在的建问题，分析并研究了光谱传输矩阵的光谱重建和散斑自相关成像的基本原理，分别对它们进行了数字和实验验证；并对光谱传输矩阵方法进行扩展，利用光谱传输方法实现散射成像；

（2）提出了一种基于三阶相关相位恢复的彩色成像方法。该项共工作的核心在于：利用三阶相关相位恢复和振幅恢复步骤的相互独立性，实现了目标方向信息的确定性恢复，实现了透过散射介质彩色成像。相较于传统的透过散射介质彩色成像方法，该方法无需对于目标的先验知识，无需介质波前整形技术，在时效性方面更具优势。同时，该方法能够与传统的光谱成像方法进行有效的结合，实现透过散射介质的光谱成像；
